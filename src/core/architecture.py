...
class LifecycleMixin:
    ...
    def system_state(self, value: LifecycleState) -> None:
        ...
        if not self._transition_to(value, warn=False):
    ...
    def _transition_to(self, new_state: LifecycleState, warn: bool = True) -> bool:
        """Безопасный переход между состояниями.

        warn=False - для legacy-сеттера system_state: там недопустимый переход
        всё равно форсируется (и логируется в debug), WARNING был бы шумом
        на каждом старте плагинов и менеджеров.
        """
