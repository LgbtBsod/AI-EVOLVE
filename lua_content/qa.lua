...
return {
  ...
  coverage = { floor = 48.3, target = 90.0, spec = 80.0 },
  hygiene = {
  ...
  tools = {
    ...
    { id = "qa_pool", purpose = "thread-pool subprocess runner: many game runs or tests in parallel with timeouts (kills the process tree)", replaces = "hand-written subprocess loops", output = "results per job", group = "libs" },
    { id = "git_util", purpose = "the one git wrapper: git(*args) -> stdout or '', head(), dirty(), changed_files(rev), git_many() for concurrent calls", replaces = "4 private git() copies in qa.py, ci, ckpt", output = "strings/lists", group = "libs" },
    { id = "jsonl_io", purpose = "the one jsonl reader/writer: read_jsonl, tail_jsonl, append_jsonl (utf-8, LF, skips corrupt or partial lines)", replaces = "hand-written json.loads(line) loops", output = "list of rows", group = "libs" },
