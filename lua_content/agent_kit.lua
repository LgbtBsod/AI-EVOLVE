...
return {
  ...
  roles = {
    ...
    { name = "reviewer", model = "sonnet", effort = "high", max_calls = 25, writes = false,
      tools = "Read, Grep, Glob, Bash",
  ...
  routes = {
    ...
    { stage = "implement",       role = "implementer", model = "sonnet", effort = "medium", max_calls = 45,
      why = "a well-scoped feature with checks that catch mistakes; the strongest model is not needed to type it" },
    { stage = "refactor",        role = "implementer", model = "sonnet", effort = "medium", max_calls = 45,
      why = "behaviour must not change and qa.py golden / test --changed prove it" },
    { stage = "rust-kernel",     role = "rust-kernel", model = "sonnet", effort = "medium", max_calls = 45,
      why = "twin plus parity fuzz pin the semantics, so the model only has to satisfy them" },
    { stage = "lua-content",     role = "lua-content", model = "sonnet", effort = "medium", max_calls = 35,
      why = "data edits checked by qa.py lua check and the item specs" },
    { stage = "fix-red-main",    role = "stabilizer",  model = "sonnet", effort = "medium", max_calls = 45,
      why = "root-causing a failure is bounded by its repro line; escalate when two causes were wrong" },
    { stage = "design",          role = "explorer",    model = "sonnet", effort = "high",   max_calls = 25,
      why = "a wrong design costs every later step: high effort on sonnet (owner rule: opus is too expensive, never route to it)" },
    { stage = "judge",           role = "reviewer",    model = "sonnet", effort = "high",   max_calls = 15,
      why = "choosing between candidate results needs judgement, and a weak judge poisons the choice" },
    { stage = "adversarial-review", role = "reviewer", model = "sonnet", effort = "high",   max_calls = 25,
