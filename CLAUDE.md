...
| Repo hygiene (tracked venv/`__pycache__`/saves/big files, `.gitignore` lost patterns) | `python tools/qa.py hygiene` (0.1 s; part of `qa.py check` and CI; lists in `hygiene` of `lua_content/qa.lua`; fix = `git rm -r --cached`) |
