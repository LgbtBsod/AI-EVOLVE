...
if not exist "%VENV_PY%" (
    echo [AI-EVOLVE] Creating virtual environment with Python 3.14...
    echo [AI-EVOLVE] ^(Panda3D has no wheels for Python 3.15 yet, so 3.14 is used^)
    py -3.14 -m venv .venv
    if errorlevel 1 (
        echo.
        echo [AI-EVOLVE] ERROR: Python 3.14 was not found via the "py" launcher.
