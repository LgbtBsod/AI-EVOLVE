...
/// Exposed to Python via `crate::ffi::PyLogCompressor`.
#[derive(Default)]
...
/// Exposed to Python via `crate::ffi::PyStateDiffCalculator`.
pub struct StateDiffCalculator;

impl StateDiffCalculator {
    pub fn calculate_diff(py: Python<'_>, old_state: &Bound<'_, PyDict>, new_state: &Bound<'_, PyDict>) -> PyResult<Py<PyDict>> {
        let diff = PyDict::new(py);

        for (key, new_val) in new_state.iter() {
            match old_state.get_item(&key)? {
                None => diff.set_item(&key, &new_val)?, // New field
                Some(old_val) => {
...
/// Exposed to Python via `crate::ffi::PyEventCorrelator`.
...
impl EventCorrelator {
    pub fn new(time_window_ms: u64) -> Self {
        Self {
            time_window_ms,
            events: Vec::new(),
        }
    ...
    pub fn add_event(&mut self, timestamp: u64, event_type: &str, payload: &str) {
    ...
    pub fn find_correlations(&self) -> Vec<String> {
...
fn is_different(old: &Bound<'_, PyAny>, new: &Bound<'_, PyAny>) -> bool {
