...
//!
//! Pure Rust; not exported to Python (no FFI wrapper in `crate::ffi` yet).
...
impl TrainingRoomEngine {
    ...
    pub fn register_effect(&mut self, effect_json: &str) -> Result<String, String> {
        let effect: EffectContract = serde_json::from_str(effect_json)
            .map_err(|e| format!("Invalid effect JSON: {}", e))?;
    ...
    pub fn create_entity(&mut self, config_json: &str) -> Result<String, String> {
    ...
    pub fn simulate(&mut self, duration: f64, tick_rate: f64) -> Result<SimulationResult, String> {
                    ...
                    if Self::check_cas_trigger(entity, effect) {
                        let event = CasEvent {
                            ...
                            context: HashMap::from([
                                ("current_hp_percent".to_string(), entity.current_hp / entity.max_hp * 100.0),
                            ]),
                        };
    ...
    fn check_cas_trigger(entity: &SimulationEntity, effect: &EffectContract) -> bool {
    ...
    pub fn get_effects(&self) -> Result<String, String> {
        serde_json::to_string(&self.effects_registry)
            .map_err(|e| format!("Serialization error: {}", e))
    }

    /// Export simulation result to JSON
    pub fn export_result(&self, result: &SimulationResult) -> Result<String, String> {
