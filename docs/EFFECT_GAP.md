...
**Status 2026-09-27 (slice F3)**: with exact aliases 55.0% (33/60, was 48.3%). Newly expressible: #39 Spider sense, #50 Prescience, #55 Shinigami eyes, #12 Kamui phase (dodge). Kinds perceive/reveal/precognition in `src/effects/perception.py`; aliases grant_vision = mod vision_range, dodge = precognition. Still blocked: #10 (copy_technique is not exact), #15 (+false_percept), #34 (reverse_causality/fate_lock: two-pass causality, a model-breaker; only its precognition part is specified). `advisor`, vision_arc, detection_range: not done (recorded as data-only later).

**Status 2026-09-27 (slice F5)**: with exact aliases 66.7% (40/60, was 58.3%). Newly expressible: #1 Infinity, #3 Domain Unlimited Void, #4 Malevolent Shrine, #16 Room / Shambles (kinds zone / zone_mod / rule_override in `src/effects/zones.py`; aliases aura, reality_marble, space_manipulation; +zone.sure_hit / +zone.on_inside provided). #32 stays blocked on `copy` (steal slice; its zone part runs), #40 stays a model-breaker (unbounded rule_override).
