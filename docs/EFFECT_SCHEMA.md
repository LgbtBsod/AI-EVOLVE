...
контекста из float) и над интерфейсом `OpHost` - ~20 примитивов `op_*`, которые даёт интерпретатор-хозяин:
...
1. `deal hp`: комната - сырой `Unit.deal_damage`; игра - конвейер `_damage` (уклонение -> крит -> броня -> iframe -> HP ->
   лайфстил -> события). `heal/set/drain`: правила потолка и воскрешения у каждого свои (`Unit` / `_heal`, `_set_resource`).
