...
## Triggers (slice F4)
- `src/effects/triggers.py`; nothing changes unless content uses it. `on_lethal` (target self, `id`, `charges` default 1 / 0 = unlimited, `priority`, `cooldown`, `duration`, `keep` hp left default 1, `restore` = % max hp instead, `ops` run on the protected entity) arms an interceptor in `unit.external["triggers"]["lethal"]`. `EffectManager._damage` asks `triggers.intercept_lethal` for a hit with amount >= hp AFTER precognition / evasion / block / invulnerable and BEFORE it is applied: the first ready interceptor by (priority, id) fires once per hit, cancels the lethal part and runs its `ops` (forms, heals, `status`). None ready -> the hit is lethal and kill / die events are unchanged. Re-entry is bounded by `MAX_EVENT_DEPTH`.
- `counter_delta` (inside on_lethal.ops; `value` = % default 50, `duration`) = `resist` to the damage type of the hit that would have killed (`triggers["last_lethal"]`).
- `delay` (`after` seconds, `ops`, `persist`): ops run when `EffectManager.update` reaches `now >= at` on the manager's game clock, queue ordered by (at, seq); a dead caster cancels it unless `persist`. Not implemented: `counter` / `reflect` / `redirect_harm` / `conditional` (planned rows), `global_time_scale`, `snapshot` / `restore_state`.
- EffectRuntime (training room) records the ops but has no damage hook or delay queue (same divergence as F1-F3). Tests: `tests/test_effect_triggers_spec.py`.

## Zones (slice F5)
- `src/effects/zones.py`; nothing changes unless content uses it (no live wiring). Ops: `zone` (`radius` required; `id`, `duration`, `affects` all|others|enemies|allies, `follow` = attached to the target entity, `to=[x,y]`, `tick {every, ops}`, `on_enter`, `on_exit`, `barrier` open|closed, `guaranteed_hit`, `shield {bypass}`, `on_inside`, `rules {consts, flags}`, `link`, `persist`), `zone_mod` (`id` + radius / affects / barrier / guaranteed_hit / duration / rules of a live zone of the caster), `rule_override` (`rules`, needs `duration` or `zone`, else refused). Aliases: `aura` = zone follow, `reality_marble` = zone marble, `space_manipulation` = zone on_inside [swap, teleport].
- Zone ops run with the OWNER as caster and the MEMBER as primary (`enemy` / `source` = member, `self` = owner). Enter / exit fire once per crossing between two `EffectManager.update` calls; ticks run per member in entity-id order on the GAME clock; O(members) per zone. Expiry, owner death (unless `persist`) or unregistration: `on_exit` for the remaining members, layers and barrier vanish.
- `barrier: closed` is enforced in `EffectManager._place` (every `move` mode) and `constrain_move(entity, x, y)`; free walking is NOT blocked until the game calls it. The owner is exempt. `guaranteed_hit`: hits by or on a member get the flag `unavoidable` and skip precognition. `shield` (Infinity): a hit on the owner by another member is nullified unless it carries `bypass` (default flag `bypass_infinity`).
- `on_inside` (`+zone.on_inside`): modes the owner may apply to any two members; `move swap with=<entity id>` swaps them and does nothing outside such a zone. `link` = portal pair (placed at the partner centre, no enter event there).
- `rules` / `rule_override`: only fields of `damage.Consts` (lua_content/damage.lua constants) and damage flags; unknown keys are dropped. A layer is dropped at its end (constants are never written). Unbounded overrides (Scarlet Witch #40) stay out: a model-breaker. `#32` `copy` is left to the steal/copy slice.
