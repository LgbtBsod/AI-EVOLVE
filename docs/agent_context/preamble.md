...
Every turn re-reads the whole context, so only two things are cheap: fewer turns and smaller results. The future agent-kit renders prompts from this file.
...
8. **Report:** at most the words the task allows; only what you ran; name what is uncertain. Off switch of the guard: `AI_EVOLVE_GUARD=off`.
