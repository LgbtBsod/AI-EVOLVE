...
| `pack` | `python tools/qa.py pack "TASK" [--files a,b] [--max-tok 1500] [--json]` | ranked context pack for a task: top files with symbol ranges, importers/tests, existing tools, state,... | 5-20 exploratory grep/ctx/Read turns | <= 1500 tokens, one grouped block | low |
...
| `relay` | `python tools/qa.py relay detect\|tick [--dry-run] [--force]\|install [--mode notify\|headless] [--every N] [--dry-run]\|uninstall\|status` | auto-continue unfinished work after a usage-limit stop or crash: OS scheduler runs `tick` (0 tokens while... | checking by hand when the usage limit resets | one line: relay mode=.. installed=.. every=.. last_tick=.. last_action=.. limit=.. | low |
...
| `relay_core` | `import relay_core` | relay_core - pure logic of `qa.py relay` (limit parsing, reset math, tick gates, command construction).... | - | - | - |
