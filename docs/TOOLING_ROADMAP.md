...
3. **agent-kit** - DONE (`lua_content/agent_kit.lua`, `tools/agent_kit.py`, `qa.py prompt` / `qa.py route`, 7 role files, generated `.claude/agents/*.md`, check `agent_kit`). OPEN: a NEW `.claude/agents/` directory needs one harness restart before the agents (and `agentType: 'implementer'` in a Workflow) appear; the `HOOK relay` markers in each role file and `planned = { "pack", "ckpt" }` in agent_kit.lua are the hook points of steps 4 and 7. Original spec: `docs/agent_context/*.md` (preamble + role files) referenced by short prompts, `qa.py prompt ROLE`, `qa.py route` (model/effort/turn budget per stage: cheap model + low effort for running tests, summarising logs, mechanical edits; strong model only for design), the `batch-rule` text, and the standard report contract (<= 250 words, fixed lines).
...
### Spec for steps 4+5 (agent-relay, ckpt, resume) - written 2026-09-26, read this instead of a long prompt

Goal: a limit crash or an agent hitting its call cap must cost ~nothing to continue (no re-exploration), and long agents are replaced by a relay of short ones (per-turn cost grows with context length).
- `qa.py ckpt "what is done" --next "next action" [--files a,b] [--agent NAME]`: appends one JSON line to `dev_probe_output/qa/journal.jsonl` (ts, head sha, dirty file list + sha256 of their content, last `check` verdict from history.jsonl, done, next, agent) and creates a SAFETY SNAPSHOT `git stash create` (never touches the working tree) stored as `refs/qa/ckpt/N` (keep the last 20, prune older). Prints one line `ckpt #N saved: dirty=D next="..."`. Thresholds/limits in `lua_content/qa.lua` (`ckpt` table).
- `qa.py resume [--brief|--restore N|--diff N]`: prints <= 6 lines and is SILENT when the tree is clean and in sync: `head=.. behind/ahead origin=..`, `dirty=N (M modified, K new)`, verdict of `qa.py check --changed` (cached result if the inputs are unchanged, else run it), ORPHANS = new untracked/added modules with no importers (the half-done-work signal, e.g. src/effects/ops.py before it was wired), BROKEN = files that fail py_compile / `luajit -bl` / cargo syntax, last journal entry (`done` / `next`), and the verdict `SAFE` (green, continue at `next`) / `RISKY` (dirty but green) / `BROKEN` (red: fix first, or `--restore N`). `--restore N` shows the diff between the working tree and ckpt N and restores ONLY on explicit `--apply`.
- Hooks (`qa.py guard --install` extended, same soft/opt-out rules): SessionStart -> prints the resume brief as additional context (only when not clean); Stop/SubagentStop -> auto `ckpt` when the tree is dirty (`--auto`, no next text).
